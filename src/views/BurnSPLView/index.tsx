...
export const BurnSPLView: FC = ({}) => {
  ...
  function SelectButton(props: { token: any }) {
    ...
    const maxAmount = props.token.amount;
    ...
    const handleSelect = () => {
      if (isSelected) {
        setIsSelected(false);
        const index = toBurn.findIndex((item: any) => item.mint === mint);
        if (index > -1) {
          toBurn.splice(index, 1);
        }
      } else {
        setIsSelected(true);
        const amount = Number(burnAmount) || maxAmount;
        toBurn.push({ tokenAccount, mint, amount });
      }
    };
    ...
    return (
      <div>
        <input
          ...
          className="input input-bordered w-[50%] max-w-xs mb-2"
        />
        <button
          className={`py-2 px-2 font-bold rounded-xl text-xs ${
            isSelected ? "bg-[#36185b] hover:bg-[#663b99]" : "bg-[#663b99] hover:bg-[#36185b]"
          } uppercase sm:ml-1 mb-2 sm:mb-4`}
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 items-start">
                          {userSPL?.map((token: any) => (
                            ...
                            >
                              ...
                              <div className="block justify-around my-2">
